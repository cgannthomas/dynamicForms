$(window).on('load', function() {
    rearrangeTableAttribute(false, true);
});
...
function rearrangeTableAttribute(is_new = false, is_page_load = false) {
    var i=0;
	$.each($('#dynamic-form tbody').find('tr'), function(index, element) {
		i++;
		$(element).attr('id', 'tr-' + i)
            .find('.field_id').attr('name', 'field[' + i + '][field_id]').end()
        ...
        if(is_page_load) {
            if (!$(element).find('.field_type').hasClass("select2-hidden-accessible")) {
                $(element).find('.field_type').select2();
            }
    ...
    if(is_new) {
        ...
        if (!$('#dynamic-form #field-type-' + i).hasClass("select2-hidden-accessible")) {
            $('#dynamic-form #field-type-' + i).select2();
        }
    }
}

$('#update-dynamic-form').submit(function(e){
    e.preventDefault();
    if(validateForm()) {
        ...
        data.append('_method', formDiv.attr('method'));
